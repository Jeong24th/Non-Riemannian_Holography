...
(**)
(*  SM (69) [SMRfirstorder]  the exact first-order rewriting of the Riemannian sigma model:*)
(*        integrating out (beta, betabar) reproduces  E_{mu nu} d x^mu dbar x^nu  with*)
(*        E = g - B, i.e. E_{+-} = 0, E_{-+} = -2F, E_{pm pm} = 2 L_pm;*)
(*  SM (70) [SMceff]  c_eff^2 = 2F, its horizon value, and  -det g_par = F^2 - 4 L+ L- = e^{-4d};*)
(*  SM (72)-(73) [SMdygconstraints, SMdygGO]  the non-Riemannian reduction: the clock*)
(*        kernel of H^{mu nu}, the dual vectors Y, Ybar, and the exact reduced Lagrangian;*)
(*        the equality of the antisymmetric-clock route SM (78) [SMdeltaL] with the*)
(*        symmetric block descent, modulo the product of the two constraints;*)
(*  SM (74) [SMGO], SM (75) [SMvertex]  the Gomis-Ooguri limit and the hair vertex*)
(*        coefficient  (1/4 pi alpha') W d x^+ dbar x^-;*)
(*  SM (76) [SMlongstringE]  the winding-string potential, including the exact identity*)
(*        Sqrt[-det g_2] = l e^{-2d} for the static winding probe;*)
(*  SM (80) [SMWZWlevel]  the flux level  k = l^2/alpha';*)
(*  SM (81)-(83) [SMFTweight, SMBRSTradial, SMBRSTmomentum]  the Fradkin-Tseytlin-improved*)
...
(* ::Section:: *)
(*SM (69)-(70): the Riemannian first-order form and c_eff*)
...
NRH`Check["SM(69): the auxiliary equations give beta = -2F d x^- and betabar = -2F dbar x^+",
...
NRH`CheckZero["SM(69): eliminating the auxiliaries reproduces E_{mu nu} dx^mu dbar x^nu, E = g - B",
   Together[L2 - LE]];
NRH`CheckZero["SM(70): -det g_par = F^2 - 4 L+ L- = e^{-4 d_R}",
   Together[-Det[{{2 Lp, -FF}, {-FF, 2 Lm}}] - (FF^2 - 4 Lp Lm)]];
NRH`CheckZero["SM(70): c_eff^2 = 2F -> 4 Sqrt[L+L-] at the horizon u = Sqrt[L+L-]",
   Together[(2 FF /. u -> Sqrt[Lp Lm]) - 4 Sqrt[Lp Lm]]];


(* ::Section:: *)
(*SM (72)-(73): the non-Riemannian clock kernel and the reduced Lagrangian*)
...
NRH`CheckZero["SM(78): symmetric-block route - antisymmetric-clock route = W (tau- . dx)(tau+ . dbar x)",
...
NRH`CheckZero["SM(74)-(75): at chi -> 0 the W coupling reduces to (W/2) dx^+ dbar x^-  (+ constraint terms)",
   Together[(symRoute /. ch -> 0 /. es -> 1) - Wc/2 (dxp + 0) (bxm + 0) - Wc/2 dxm bxp]];
NRH`Check["SM(75): with the 1/(2 pi alpha') prefactor this is V_W = (1/(4 pi alpha')) W dx+ dbar x-",
   Together[1/(2 Pi ap) Wc/2 - Wc/(4 Pi ap)] === 0];


(* ::Section:: *)
(*SM (76): the static winding probe*)
...
NRH`CheckZero["SM(76): Sqrt[-det g_2] = l w e^{-2d}   (exact, arbitrary chiral L_pm)",
   Together[-Det[g2] - (l wN (u - Lp Lm/u))^2]];
(* B_{t phi} = l (e^{2y/l} + L+ L- e^{-2y/l}) per winding unit *)
BtphiPerW = l (u + Lp Lm/u);
NRH`CheckZero["SM(76): E(y) = (w l/alpha')[e^{-2d} - (e^{2y/l} + L+L- e^{-2y/l})] = -(2 w l/alpha') L+L- e^{-2y/l}",
   Together[wN l/ap ((u - Lp Lm/u) - (u + Lp Lm/u)) + 2 wN l/ap Lp Lm/u]];


(* ::Section:: *)
(*SM (80): the flux level*)
...
NRH`CheckZero["SM(80): k = |Int_{S^3} H| / (4 pi^2 alpha') = l^2/alpha'",
   Together[(-fluxH)/(4 Pi^2 ap) - l^2/ap]];


(* ::Section:: *)
(*SM (81)-(83): the Fradkin-Tseytlin radial weight*)
...
hy = -ap/4 aa (aa + 2/l);
NRH`CheckZero["SM(81): the two OPE contributions assemble to  h_y(a)/(z-w)^2",
   Together[doubleContraction + improvement - hy/(z - w)^2]];
NRH`CheckZero["SM(82): the marginal roots of h_y are a = 0 and a = -2/l  (modes {1, e^{-2y/l}})",
   {hy /. aa -> 0, hy /. aa -> -2/l}];
NRH`CheckZero["SM(82): (d_y^2 + (2/l) d_y) f = 0  for  f = W0 + W1 e^{-2y/l}",
   Module[{f = W0c + W1c Exp[-2 yy/l]}, Together[D[f, {yy, 2}] + 2/l D[f, yy]]]];
NRH`CheckZero["SM(83): h_y(i k) = (alpha'/4) k (k - 2 i/l) and P = k - i/l gives (alpha'/4)(P^2 + 1/l^2)",
...
(* ::Text:: *)
(*In the Gomis-Ooguri system SM (74) the only singular contractions are*)
